	...
	switch (action.type) {
		...
		case Actions.ChangeTab:
			return {
				...state,
				tab: action.tab,
				refreshCompare: action.tab === Tab.Compare ? true : state.refreshCompare
			};
		...
		case Actions.ToggleSelected:
			if (action.action === 'add') {
				...
				for (let i = 0; i < state.selectedCompare.length; i++) {
					const id = state.selectedCompare[i];
